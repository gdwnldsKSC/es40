...
void CAliM1543C::check_state()
{
	...
	// Read current value
	u64 cur = cSystem->ReadMem(ctb_turboslot_phys, 64, this);  // CSystem::ReadMem is exposed to devices. :contentReference[oaicite:3]{index=3}
	...
	if ((cur & 0x000000000000FFFFULL) == 0x000000000000FFFFULL) {
		...
		cSystem->WriteMem(ctb_turboslot_phys, 64, ts, this);   // signature: WriteMem(addr, dsize, data, source). :contentReference[oaicite:4]{index=4}
