...
static u8   g_cmd = 0;
static bool g_write_enable = false;
	...
	case 3:
		if (g_cmd == 0xA0) {
			if (g_write_enable) {
				...
				printf("[TIGFLASH] program blocked (fw-write disabled)\n");
			}
			g_cycle = 0; g_cmd = 0;
			break;
		}
	...
	case 5:
		if (is_5555(a) && val == 0x10) {
			if (g_write_enable) {
				...
				printf("[TIGFLASH] chip erase blocked (fw-write disabled)\n");
			}
			g_cycle = 0; g_cmd = 0; 
			break;
		}
		if (val == 0x30) {
			if (g_write_enable) {
				...
				printf("[TIGFLASH] sector erase blocked (fw-write disabled)\n");
			}
			g_cycle = 0; g_cmd = 0; 
			break;
		}
