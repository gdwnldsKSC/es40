...
{
	if (state.CRTC.reg[0x34] & 0x80) {
		// Lock clock select (bits 3:2) to current setting
		value = (value & ~0x0C) | ((state.misc_output.clock_select & 0x03) << 2);
#if DEBUG_VGA_NOISY
		printf("3C2[3:2] change blocked by CR34[7]\n");
#endif
	}
...
void CS3Trio64::write_b_3c5(u8 value)
{
	...
	case 0x01: { // Sequencer: clocking mode
		u8 newval = value & 0x3f;
		if (state.CRTC.reg[0x34] & 0x20) {
			// LOCK 8/9D: preserve bit0 (8/9-dot) from the previous value
			newval = (newval & ~0x01) | (state.sequencer.reg1 & 0x01);
#if DEBUG_VGA_NOISY
			printf("SR01 bit0 change blocked by CR34[5]\n");
#endif
		}
		state.sequencer.reg1 = newval;
		break;
	}
...
#if DEBUG_VGA_NOISY
		printf("DAC write blocked by CR33[4]\n");
#endif
		return;
	}
...
void CS3Trio64::write_b_3d4(u8 value)
{
	...
	if ((state.CRTC.address > 0x18) && (state.CRTC.address != 0x38) && (state.CRTC.address != 0x39) && (state.CRTC.address != 0x2e)
		...
		&& (state.CRTC.address != 0x60) && (state.CRTC.address != 0x32) && (state.CRTC.address != 0x33) && (state.CRTC.address != 0x34))
	{
...
void CS3Trio64::write_b_3d5(u8 value)
{
	...
	if ((state.CRTC.address > 0x18) && (state.CRTC.address != 0x38) && (state.CRTC.address != 0x39) && (state.CRTC.address != 0x2e)
		...
		&& (state.CRTC.address != 0x5d) && (state.CRTC.address != 0x5e) && (state.CRTC.address != 0x60) && (state.CRTC.address != 0x32)
		&& (state.CRTC.address != 0x33) && (state.CRTC.address != 0x34))
	{
	...
	if (value != state.CRTC.reg[state.CRTC.address])
	{
		...
		case 0x32: { // Backwards Compatibility 1
			u8 old = state.CRTC.reg[0x32];
			state.CRTC.reg[0x32] = value;

			// If CK-CLK bits changed or force-high-char-clock toggled,
			// a text width / timing re-eval is needed.
			if (((old ^ value) & 0x07) != 0) {
				redraw_area(0, 0, old_iWidth, old_iHeight);
			}
			break;
		...
		case 0x33: { // Backwards Compatibility 2
			u8 old = state.CRTC.reg[0x33];
			state.CRTC.reg[0x33] = value;

			// If palette or DAC lock changed, we don't need a redraw immediately,
			// but it affects future writes. No-op here.
			(void)old;
			break;
		}

		case 0x34: { // Backwards Compatibility 3
			u8 old = state.CRTC.reg[0x34];
			state.CRTC.reg[0x34] = value;

			// If LOCK 8/9 (bit5) changed, text width may effectively lock/unlock -> redraw.
			if (((old ^ value) & 0x20) != 0) {
				redraw_area(0, 0, old_iWidth, old_iHeight);
			}
			break;
...
void CS3Trio64::update(void)
{
	...
	{                     // text mode
		...
		u8 cr32 = state.CRTC.reg[0x32];
		unsigned ckclk = (cr32 & 0x03);
		unsigned cWidthIBM = ((state.sequencer.reg1 & 0x01) ? 8 : 9);

		switch (ckclk) {
		case 1:  cWidth = 7;         break;  // force 7-dot chars
		case 2:  cWidth = 9;         break;  // force 9-dot chars
		default: cWidth = cWidthIBM; break;  // IBM semantics (8 or 9 per SR01)
		}
...
void CS3Trio64::vga_mem_write(u32 addr, u8 value)
{
	...
	if (state.CRTC.reg[0x32] & 0x40) { // CR32 bit6: “Fix VGA screen page…”
		u32 hi = (state.CRTC.reg[0x31] >> 4) & 0x3; // CR31[5:4] -> bits 17:16
		start_addr |= (hi << 16);
	}
